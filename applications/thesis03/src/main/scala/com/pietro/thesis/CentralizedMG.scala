...
object CentralizedMGSummary {
  def main(args: Array[String]) {

    val prop = new Properties()
    prop.load(new FileInputStream("app.properties"))
    val clusterId = prop.getProperty("cluster.id")
    val clusterName = prop.getProperty("centralized.metricsPrefix")
    ...
    val inputTopic = prop.getProperty("centralized.input.topic")
	...
	.option("kafka.bootstrap.servers", outputBrokers)
    	...
    	.option("checkpointLocation", "centralized-cl") // ComputeClusterLocal-StructuredStreaming
    	.start()
	...
	val sizeMetric = s"""${clusterName}.summary.size ${size} ${time}"""
	val errorMetric = s"""${clusterName}.summary.error ${error} ${time}"""
	val correctValuesMetric = s"""${clusterName}.summary.correctValues ${correctValues} ${time}"""
