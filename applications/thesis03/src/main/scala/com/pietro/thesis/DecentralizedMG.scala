...
//        .selectExpr("CAST(window AS STRING) AS key", "to_json(struct(*)) AS value") // Prepare to write back to Kafka
	...
	/* When posted: Right before StreamExecution finishes running streaming batches (due to a stop or an exception) */
		...
		val numInputRows = s"""decentralized.${appId}.queryProgress.numInputRows ${queryProgress.progress.numInputRows} ${time}"""
		out.println(numInputRows)
		out.flush

		val inputRowsPerSecond = s"""decentralized.${appId}.queryProgress.inputRowsPerSecond ${queryProgress.progress.inputRowsPerSecond} ${time}"""
		out.println(inputRowsPerSecond)
		out.flush

		val processedRowsPerSecond = s"""decentralized.${appId}.queryProgress.processedRowsPerSecond ${queryProgress.progress.processedRowsPerSecond} ${time}"""
...
//      socket = new Socket(InetAddress.getByName("sky2.it.kth.se"), 2003)
      ...
      override def process(value: Row) = {
        ...
        val sizeMetric = s"""decentralized.${appId}.decSummary.size ${size} ${time}"""
        val errorMetric = s"""decentralized.${appId}.decSummary.error ${error} ${time}"""
        val accuracyMetric = s"""decentralized.${appId}.decSummary.accuracy ${accuracy} ${time}"""
        val stddevMetric = s"""decentralized.${appId}.decSummary.stddev ${stddev} ${time}"""
        ...
        println(sizeMetric)
        println(errorMetric)
        println(accuracyMetric)
	println(stddevMetric)
